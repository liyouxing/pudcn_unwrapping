...
class PUDCN(torch.nn.Module):
    ...
    def forward(self, x):
        # coarse stage encoding
        x1 = self.init_block(x)
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x4 = self.down3(x3)

        y4 = self.block(x4)  # fuse feats 4

        # coarse stage decoding
        y3 = self.dec3(torch.cat([self.up3(y4), x3], 1))  # fuse feats 3
        y2 = self.dec2(torch.cat([self.up2(y3), x2], 1))  # fuse feats 2
        y1 = self.coarse_block(torch.cat([self.up1(y2), x1], 1))
        coarse_out = self.coarse_end(y1)

        # fine stage
        # pass
        return coarse_out, coarse_out, coarse_out
